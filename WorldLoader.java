...
public class WorldLoader {
	...
	public void load(World world) {
		...
		try {
			...
			Document document = builder.build("maps/dump.xml");
	...
	public void saveLayer1(World world) {
		...
		for (int x = 0; x < 26 - 1; x++) {
			for (int y = 0; y < 20 - 1; y++) {
				...
				block.setAttribute("xPos", String
						.valueOf((int) world.currentGrid[x][y][1].getxPos()));
				block.setAttribute("yPos", String
						.valueOf((int) world.currentGrid[x][y][1].getyPos()));
				block.setAttribute("type",
						String.valueOf((int) world.currentGrid[x][y][1].type));
				root.addContent(block);
				}
			}
		}
